...
const AVAILABLE_SPACE = TOTAL_SPACE - allDirectories[0].size
const NEEDED_SPACE = MINIMUM_SPACE - AVAILABLE_SPACE
const sortedSizes = allDirectories.map(dir => dir.size).sort((a, b) => a - b)
const result = sortedSizes.find(size => size > NEEDED_SPACE)
